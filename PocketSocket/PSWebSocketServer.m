...
- (void)notifyDelegateConnection:(PSWebSocketServerConnection *)connection didFailWithError:(NSError *)error {
    [self executeDelegate:^{
        if ([self->_delegate respondsToSelector: @selector(server:connectionId:didFailWithError:)]) {
            [self->_delegate server:self connectionId:connection.identifier didFailWithError:error];
        }
    }];
}
...
- (NSHTTPURLResponse *)responseFromDelegateForConnection:(PSWebSocketServerConnection *)connection
    ...
    [self executeDelegateAndWait:^{
        if([self->_delegate respondsToSelector:@selector(server:responseOnSimpleGetRequest:address:trust:responseBody:)]) {
            NSData* address = PSPeerAddressOfInputStream(connection.inputStream);
            SecTrustRef trust = (SecTrustRef)CFReadStreamCopyProperty(
                                                                      (__bridge CFReadStreamRef)connection.inputStream,
                                                                      kCFStreamPropertySSLPeerTrust);
            response = [self->_delegate server:self
